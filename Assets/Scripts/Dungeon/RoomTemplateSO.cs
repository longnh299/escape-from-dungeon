...
public class RoomTemplateSO : ScriptableObject
{
    ...
    #region Tooltip
    [Tooltip("The gameObject prefab for the room")]
    #endregion Tooltip
    public GameObject prefab;

    // use to regenerate the guid if the so is copied and the prefab is changed
    ...
    [Tooltip("The room node type SO. The room node types correspond to the room nodes used in the room node graph.  " +
        ...
        " For the room templates there are 2 corridor node types - VerticalCorridor and HorizontalCorridor.")]
    #endregion Tooltip
    ...
    {
        // set unique guid if empty or prefab changed
        ...
        // check doorway list
        HelperUtilities.IsEnumerableValues(this, nameof(doorwayList), doorwayList);

        // check spawn postions populated
        HelperUtilities.IsEnumerableValues(this, nameof(spawnPositionArray), spawnPositionArray);
