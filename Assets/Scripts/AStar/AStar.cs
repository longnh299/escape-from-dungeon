...
public static class AStar
{
    ...
    public static Stack<Vector3> BuildPath(Room room, Vector3Int startGridPosition, Vector3Int endGridPosition)
    {
        ...
        // currentNode has parent node, and previous previous...
        Node endPathNode = FindShortestPath(startNode, targetNode, gridNodes, openNodeList, closedNodeHashSet, room.instantiatedRoom);

        if (endPathNode != null)
        {
            return CreatePathStack(endPathNode, room); // create path from startNode to endNode
        }
    ...
    private static Node FindShortestPath(Node startNode, Node targetNode, GridNodes gridNodes, List<Node> openNodeList, HashSet<Node> closedNodeHashSet, InstantiatedRoom instantiatedRoom)
    {
        ...
        while (openNodeList.Count > 0)
        {
            ...
            openNodeList.RemoveAt(0); // remove currentNode from openNodeList

            // if the current node = target node then finish => finding successfully
    ...
    private static Stack<Vector3> CreatePathStack(Node targetNode, Room room)
    {
        ...
        while (nextNode != null)
        {
            ...
            nextNode = nextNode.parentNode; // previous of target node in shortest path
        }
    ...
    {
        Vector2Int currentNodeGridPosition = currentNode.gridPosition; // gridPosition is (x, y)

        Node validNeighbourNode;

        // Loop through all directions (8 neighbour node)
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                // get valid nodeNeighbour of curentNode
    ...
    private static Node GetValidNodeNeighbour(int neighbourNodeXPosition, int neighbourNodeYPosition, GridNodes gridNodes, HashSet<Node> closedNodeHashSet, InstantiatedRoom instantiatedRoom)
    {
        ...
        // check for obstacle at that position, if movementPenaltyForGridSpace = 1 => can move, = 0 => can not move
