...
public class EnemyMovementAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        playerReferencePosition = enemy.GetPlayer().position;
    ...
    private void MoveEnemy()
    {
        ...
        if (!chasePlayer && Vector3.Distance(transform.position, enemy.GetPlayer().position) < enemy.enemyDetails.chaseDistance)
        {
            chasePlayer = true;
        }
        ...
        if (currentEnemyPathRebuildCooldown <= 0f || (Vector3.Distance(playerReferencePosition, enemy.GetPlayer().position) > Settings.playerMoveDistanceToRebuildPath))
        {
            // Reset path rebuild cooldown timer
            currentEnemyPathRebuildCooldown = Settings.enemyPathRebuildCooldown;

            // Reset player reference position
            playerReferencePosition = enemy.GetPlayer().position; // get player position
    ...
    private void CreatePath()
    {
        ...
        // Get players position on the grid in room
...
// Set the frame number that the enemy path will be recalculated on - to avoid performance spikes
    ...
    {
        Vector3 playerPosition = enemy.GetPlayer().position;
